...
export async function generateMetadata({ params: { locale } }: GamesProps): Promise<Metadata> {
  const { t } = await createTranslation(locale, 'games')
...
export default async function Games({ params: { locale } }: GamesProps) {
  const { t } = await createTranslation(locale, 'games')
  return (
